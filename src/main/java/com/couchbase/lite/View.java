...
public final class View {
    ...
    @SuppressWarnings("unchecked")
    @InterfaceAudience.Private
    public void updateIndexOptimized() throws CouchbaseLiteException {
        Log.v(Log.TAG_VIEW, "Re-indexing view: %s", name);
        assert (mapBlock != null);
        ...
        try {
            ...
            while (keepGoing) {
                ...
                String revId = cursor.getString(3);
                byte[] json = null;//cursor.getBlob(4);

                boolean noAttachments = cursor.getInt(4) > 0;//cursor.getInt(5) > 0;
                boolean deleted = cursor.getInt(5) > 0;//cursor.getInt(6) > 0;
                ...
                if(json == null) {
                    String[] sqlArgs = {Long.toString(sequence)};
                    json = Utils.byteArrayResultForQuery(database.getDatabase(), "SELECT json FROM revs WHERE sequence=?", sqlArgs);
                }
    ...
    @SuppressWarnings("unchecked")
    @InterfaceAudience.Private
    public void updateIndex(int batchCount) throws CouchbaseLiteException {
        Log.v(Log.TAG_VIEW, "Re-indexing view: %s", name);
        assert (mapBlock != null);
        ...
        try {
            ...
            StringBuffer subRequest = new StringBuffer("SELECT DISTINCT revs.doc_id, docid, max(sequence) AS sequence FROM revs, docs WHERE revs.doc_id=docs.doc_id AND sequence>? AND current!=0 ");
            if (minLastSequence == 0)
                subRequest.append("AND deleted=0 ");
            subRequest.append("GROUP BY revs.doc_id, docid");
            StringBuffer request = new StringBuffer("SELECT revs.doc_id, revs.sequence, docid, revid, json, no_attachments, deleted FROM revs, (");
            request.append(subRequest);
            request.append(") AS tmp WHERE revs.sequence = tmp.sequence ORDER BY revs.sequence ASC LIMIT ?");

            long currentMinSequence = minLastSequence;
            boolean processNextBatch = true;
            while (processNextBatch) {
                // Now scan every revision added since the last time the view was
                // indexed:
                String[] selectArgs = {Long.toString(currentMinSequence), Integer.toString(batchCount)};
                cursor = database.getDatabase().rawQuery(request.toString(), selectArgs);
